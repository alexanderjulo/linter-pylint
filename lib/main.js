  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        let execPath = fixPathString(this.executablePath, '', projectDir);
        if (!fs().existsSync(execPath) && execPath != 'pylint') {
          console.log(`Could not find ${execPath}, trying to fall back to pylint...`);
          execPath = 'pylint';
        }
